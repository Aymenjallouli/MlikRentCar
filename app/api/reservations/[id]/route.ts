...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const updated = await updateReservationStatus(params.id, status, adminNote);
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  if (!requireAdmin(req)) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  const reservations = await getReservations();
  const filtered = reservations.filter((r) => r.id !== params.id);
  if (filtered.length === reservations.length) return NextResponse.json({ error: "Not found" }, { status: 404 });
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return NextResponse.json({ ...r, car: await getCarById(r.carId) });
}
